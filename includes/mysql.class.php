...
function connect($new_link = false) {
	...
	$this->con_id = new mysqli($this->server, $this->user, $this->pass);

	if ($this->con_id->connect_errno) {	// connect error
		$this->oops("Could not connect to server: <b>$this->server</b>.");
	}

	if (!$this->con_id->select_db($this->database)) {		//no database
...
# Param: string
# returns: string
function log_mysql($string, $filename = "mysql_log.txt") {
...
function query($sql) {
	// do query
	$this->query_id = $this->con_id->query($sql);
	...
	$this->affected_rows = $this->con_id->affected_rows;
...
function free_result($query_id = -1) {
	...
	$this->query_id->free_result();
	
	// if($this->query_id != 0 && !$this->query_id->free_result()) {
...
function query_update($table, $data, $where='1') {
	...
	if (count($data) > 0) {
		foreach($data as $key=>$val) {
			...
			elseif(preg_match("@AES_ENCRYPT\s*\(\s*'([^']+)'\s*,\s*'([^']+)'\s*\)@i", $val, $match)) {
				$q.= "`$key`=AES_ENCRYPT('".$this->escape($match[1])."', '".$this->escape($match[2])."'), ";
			}
			else {
...
function query_insert($table, $data) {
	...
	if (count($data) > 0) {
		foreach($data as $key=>$val) {
			...
			elseif(preg_match("@AES_ENCRYPT\s*\(\s*'([^']+)'\s*,\s*'([^']+)'\s*\)@i", $val, $match)) {
				$v.= "AES_ENCRYPT('".$this->escape($match[1])."', '".$this->escape($match[2])."'), ";
			}
			else {
				$v.= "'".$this->escape($val)."', ";
			}
