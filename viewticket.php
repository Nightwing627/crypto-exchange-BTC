...
//    status, isanswer closedate
    $db->query_update(TABLE_TICKETS, $close, "ticketid = '" . $u_ticket_id . "' and user_id='" . $_SESSION['user_id'] . "'");
...
$sql = "SELECT ticketid, subject, description, department, urgency, status, isanswer, if(isanswer = 1, 'Answered', 'No Answered') answered, opneddate, date_format(opneddate, '%d-%m-%Y %H:%i:%s') str_opneddate , date_format(closeddate, '%d-%m-%Y %H:%i:%s') str_closeddate, user_id FROM `" . TABLE_TICKETS . "` WHERE user_id = '" . $_SESSION['user_id'] . "' and ticketid = '" . $u_ticket_id . "' ORDER BY opneddate DESC";
$ticket_history = $db->fetch_all_array($sql);
$ms_sql = "select  description, date_format(posttime, '%d-%m-%Y %H:%i:%s') str_posttime, user_name  from answers s inner join users u on u.user_id = s.user_id "
        . " where s.ticketid = " . $u_ticket_id . " and s.user_id = " . $_SESSION['user_id'] . " order by posttime desc";
...
<div id="support">
    ...
    <div class="right-menu">
        ...
        if ($status != "Closed") {
            ?>
            <form method="post" class="form narrow-cols" action="viewticket?common=<?= $_REQUEST['common'] ?>">
                ...
                <div class="form-buttons">
                    ...
                    <a class="button_link" href="viewticket?rem=y&common=<?= $_REQUEST['common'] ?>">Close Ticket</a>
                </div>
            </form>
        <?php } ?>
    </div>
